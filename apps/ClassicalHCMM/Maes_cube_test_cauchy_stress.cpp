...
template <int dim, typename Number = double>
class NeumannBoundary : public dealii::Function<dim, Number>
{
public:
  NeumannBoundary(const std::vector<Number> &values, const unsigned int n_steps)
    : dealii::Function<dim, Number>(dim)
    , values(values)
    , n_steps(n_steps)
  {}
        ...
        const unsigned int        component = 0) const override
  {
    ...
    if (this->get_time() >= n_steps)
      return 20. + values[component] * n_steps;
    else
      return 20. + values[component] * this->get_time();
  }
...
template <int dim, typename Number = double>
class SpatialNeumannBoundaryPrestretch : public dealii::Function<dim, Number>
{
public:
  SpatialNeumannBoundaryPrestretch(const Number base_pressure,
                                   const Number pressure_increment,
                                   const Number t_end)
    : dealii::Function<dim, Number>(dim)
    , base_pressure(base_pressure)
        ...
        const unsigned int        component = 0) const override
  {
    ...
    if (this->get_time() >= t_end)
      return base_pressure + pressure_increment * t_end * (1.0 + 0.0 * p[0]);
    else
      return base_pressure +
             pressure_increment * (1.0 + 0.0 * p[0]) * this->get_time();
  }
...
main(int argc, char *argv[])
{
  ...
  const std::string filename = "Maes_cube_test_cauchy_stress_model_E_case_S";
  // prestretch
  const std::string prestretched_configuration =
    "../Prestretching/Maes_cube/Maes_cube_test_prestretch_cauchy_stress.mesh";
  ...
  const double              applied_stress = 0.12; // [MPa]
  ...
  try
    {
      ...
      boundary_descriptor_HCMM.add_pressure_bc(
        3,
        std::make_shared<SpatialNeumannBoundaryPrestretch<dim>>(
          0.0, applied_stress / n_load_steps, n_load_steps));
      // create Mixture
