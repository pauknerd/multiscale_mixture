...
main(int argc, char *argv[])
{
  ...
  const std::string filename = "Latorre_cylinder_test_quadratic_k_04_dp_15p";
  // prestretch
  ...
  // material parameters Latorre
  const double              pressure = 1.15 * 14.0; // Latorre
  const std::vector<Number> mass_fractions{
    0.34, 0.056 * 0.33, 0.067 * 0.33, 0.4385 * 0.33, 0.4385 * 0.33, 0.33};

  // ELASTIN
  const Number E_elastin = 89.71;
  const Number kappa     = 10.0 * E_elastin;

  // SMC
  const Number k_1_smc        = 261.4;
  const Number k_2_smc        = 0.24;
  const Number prestretch_smc = 1.2;
  const Number T_smc          = 70.;         // decay time [days]
  const Number k_smc          = 0.4 / T_smc; // gain factor

  // COL
  const double k_1_col        = 234.9;
  const double k_2_col        = 4.08;
  const double prestretch_col = 1.25;
  const Number T_col          = 70.;         // decay time [days]
  const Number k_col          = 0.4 / T_col; // gain factor
  const Number alpha          = 0.52202798;  // fiber orientation
