...
{
public:
  explicit InitialConditionMATLABFit(const unsigned int n_components)
...
main(int argc, char *argv[])
{
  ...
  //// PARAMETERS
  const std::string output_directory = "Latorre_cylinder_split_EC/";
  const std::string filename =
    "Latorre_cylinder_test_split_EC_quadratic_s_stress_05_s_NO_075_s_ET_075_s_WSS_05_dp_10p_dQ_0p";
  // prestretch
  ...
  const bool         active_GR                = true; // false;
  ...
  const double       equilibration_time =
    100.0; // final time in pathway solver, should be high enough to reach
           // steady state
  ...
  const double k_NO =
    1440. * 4.0; // degradation time constant NO (1/15s = 86400/15 1/days)
  ...
  const std::vector<double> mass_fractions                  = {
    ...
    0.33};                                     // SMC (circ)
  // perturbations
  const double pressure_increase = 1.1; // fold increase in pressure
  const double flow_increase     = 1.0; // fold increase in flow

  const double pressure = pressure_increase * 14.0; // new pressure
  ...
  try
    {
      ...
      // solved first step of problem once to get initial distribution of NO and
      // ET1
      ...
      auto endothelial_input_transformer =
        ...
        // note: not correct to use l2_norm since that also turns negative
        // displacement into a positive change in radius determine flow rate
        // fold change
