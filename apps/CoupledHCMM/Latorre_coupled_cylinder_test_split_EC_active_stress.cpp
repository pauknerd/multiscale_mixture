...
#include <MSHCMM/pathways/equations/Irons2020_EC_pathway_extracted.h>
#include <MSHCMM/pathways/equations/Irons2020_SMC_pathway_extracted.h>
...
class InitialConditionMATLABFit : public dealii::Function<dim, Number>
{
public:
  InitialConditionMATLABFit(const unsigned int n_components)
        ...
        const unsigned int        component) const override
  {
    if (component == 0)
      ...
      {
        // compute radius
        const Number radius = std::sqrt(p[0] * p[0] + p[1] * p[1]);
        // coefficients obtained through fit in MATLAB using curve fitting tool
        // with a quadratic function
        return p_1_NO * radius * radius + p_2_NO * radius + p_3_NO;
      }
    else if (component == 3)
      {
        // compute radius
        ...
        // coefficients obtained through fit in MATLAB using curve fitting tool
        // with a quadratic function
        return p_1_ET1 * radius * radius + p_2_ET1 * radius + p_3_ET1;
      }
    else
      return 0.0;
  }
...
private:
  // coefficients NO
  const double p_1_NO = 0.004012094144408;
  const double p_2_NO = -0.008392326370477;
  const double p_3_NO = 0.003983856184798;
  // coefficients ET1
  const double p_1_ET1 = 0.008335061453380;
  const double p_2_ET1 = -0.017429420905095;
  const double p_3_ET1 = 0.008176006588133;
};
...
main(int argc, char *argv[])
{
  ...
  //// PARAMETERS
  const std::string output_directory = "Latorre_cylinder_split_EC_active/";
  const std::string filename =
    "Latorre_cylinder_test_split_EC_active_stress_quadratic_s_stress_05_s_con_1_s_NO_075_s_ET_075_s_WSS_05_dp_0p_dQ_10p";
  // prestretch
  const std::string prestretched_configuration =
    "../Prestretching/Latorre_cylinder_active/Latorre_cylinder_test_prestretch_active_quadratic.mesh";
  ...
  // cylinder 1
  const std::vector<double> initial_condition_diffusion = {
    0.33, 0.33, 0.0003, 0.0003}; // collagen, SMC, NO, ET1; no elastin

  const std::vector<std::string> diffusion_component_names = {"collagen",
                                                              "SMC",
                                                              "NO",
                                                              "ET1"};
  const std::vector<Number>      D_coefficients{0.0, 0.0, 285.0, 25.92};
  ...
  const double k_NO  = 1440. * 4.0;       // degradation time constant NO
  const double k_ET1 = 1.0 * 1440. / 2.0; // degradation time constant ET1
  ...
  const std::vector<double> mass_fractions                  = {
    ...
    0.33};                                     // SMC (circ)
  // perturbations
  const double pressure      = 1.0 * 14.0; // fold increase in pressure
  const double flow_increase = 1.1;        // fold increase in flow
  // geometry data
  const double L              = 0.04;
  ...
  // active stress
  const double                      lambda_max = 1.1;   // 1.4;
  const double                      lambda_min = 0.4;   // 0.8;
  const double                      sigma_max  = 170.0; // 54.0;
  const double                      density    = 1.0;
  Materials::ActiveMaterialSettings active_material_settings{sigma_max,
                                                             density,
                                                             lambda_min,
                                                             lambda_max};
  ...
  const double sensitivity_NO     = 0.75;
  const double sensitivity_ET1    = 0.75;
  const double s_contractility =
    1.0; // sensitivity of contractile stress to changes in pathway output

  const double y_0_stress    = 0.2;
  const double y_0_NO        = 0.5431;
  const double y_0_ET1       = 0.1707;
  const double y_0_AngII     = 0.0;
  const double y_0_SACs      = 0.2;
  const double y_0_integrins = 0.2;

  // ECs
  const double sensitivity_WSS = 0.5;
  ...
  try
    {
      ...
      //      std::make_shared<Functions::ConstantFunction<dim>>(initial_condition_diffusion);
      auto initial_mass_fractions_diffusion =
        std::make_shared<InitialConditionMATLABFit<dim>>(4);
      ...
      constituent_factories.push_back(
        std::make_unique<Constituents::ConstituentFiberFactory<dim, Number>>(
          ...
          [k_1_smc, k_2_smc, cos_transformer_1, active_material_settings](
            const Point<dim, Number> &point) {
            (void)point;
            const auto fiber_orientation = cos_transformer_1.get_tangent(point);

            return std::make_unique<Materials::FiberMaterial_1D<dim, Number>>(
              k_1_smc, k_2_smc, fiber_orientation, active_material_settings);
          },
          time_step_size_HCMM,
          T_smc,
          k_smc));
      ...
      auto HCMM_col_circ_transformer =
        ...
        transferable_parameters.current_mass_fraction =
          diffusion_values[0] * relative_collagen_fraction;
        ...
        transferable_parameters.mass_production =
          (k_col * initial_mass_fractions[0] / initial_mass_fractions[1] *
             (1.0 + delta_psi_col) * diffusion_values[1] +
           D_coefficients[0] * diffusion_laplacians[0]) *
      ...
      /// constituent 5
      const auto contractility_scaler =
        Scalers::exponential(s_contractility, 0.0, 1.0);

      // Circ SMCs: constituent that produces other constituents
      auto HCMM_smc_transformer =
        [k_smc, D_coefficients, contractility_scaler](
        ...
        // "classical", uncoupled version is used which does not care about that
        // since it doesn't need transformers.
        transferable_parameters.current_mass_fraction = diffusion_values[1];
        ...
        transferable_parameters.mass_production =
          k_smc * (1.0 + delta_psi_smc) * diffusion_values[1] +
          D_coefficients[1] * diffusion_laplacians[1];

        // set contractility
        // compute normalized change in pathway output "contractility"
        Number delta_psi_contractility =
          (average_pathway_output[2] - average_baseline_pathway_output[2]) /
          average_baseline_pathway_output[2];
        transferable_parameters.contractility = contractility_scaler(
          delta_psi_contractility); // time > 0. ? 1.0 : 0.5;
      };
      ...
      auto diffusion_transformer =
        ...
        linear_matrix[1][1] = (k_smc * (1.0 + delta_psi_smc) - k_smc);
        // collagen mass fraction
        // constant degradation
        linear_matrix[0][0] = -k_col;
        ...
        linear_matrix[0][1] =
          mass_fractions[0] / mass_fractions[1] * k_col * (1.0 + delta_psi_col);
        // degradation of NO
        linear_matrix[2][2] = -k_NO;
        // degradation of ET1
        linear_matrix[3][3] = -k_ET1;
      };
      ...
      // equation in the concrete class!!!
      pathwayEquations.push_back(
        std::make_shared<Equations::Irons2020SMCPathway<Number>>());
      ...
      // that pathway which the lambda should capture.
      const auto exp_scaler_stress =
        Scalers::NEW_exponential(sensitivity_stress, y_0_stress);
      const auto exp_scaler_NO =
        Scalers::NEW_exponential(sensitivity_NO, y_0_NO);
      const auto exp_scaler_ET1 =
        Scalers::NEW_exponential(sensitivity_ET1, y_0_ET1);

      const unsigned int pathway_index_of_stress = 0;
      const unsigned int pathway_index_of_ET1    = 1;
      const unsigned int pathway_index_of_NO     = 2;
      ...
      auto pathway_transformer =
        ...
        //// stress input
        cell_state[pathway_index_of_stress] = exp_scaler_stress(delta_sigma);

        //// diffusion input
        // get initial values of NO and ET1
        initial_mass_fractions_diffusion->vector_value(p,
                                                       initial_concentrations);
        // NO
        const auto delta_NO =
          (diffusion_values[2] - initial_concentrations[2]) /
          initial_concentrations[2];
        cell_state[pathway_index_of_NO] = exp_scaler_NO(delta_NO);

        // ET1
        const auto delta_ET1 =
          (diffusion_values[3] - initial_concentrations[3]) /
          initial_concentrations[3];
        cell_state[pathway_index_of_ET1] = exp_scaler_ET1(delta_ET1);

        // other inputs are fixed
        cell_state[3] = y_0_AngII;     // AngII
        cell_state[4] = y_0_SACs;      // SACs
        cell_state[5] = y_0_integrins; // Integrins
      };
      ...
      //// Endothelial Cells
      const unsigned int endothelial_boundary_id = 0;
      std::vector<std::shared_ptr<Equations::PathwayEquationBase<Number>>>
        endothelial_pathways;
      // pathway weights for endothelial cells, trivial since only once pathway
      // equation
      const std::vector<Number> endothelial_pathway_weights{1.0};
      // add pathway equation
      endothelial_pathways.push_back(
        std::make_shared<Equations::Irons2020ECPathway<Number>>());
      ...
      // cell type, we can use the passed pathway_id of the passed cell_state to
      // the function. Then we can use that to select the appropriate scaler for
      // that pathway which the lambda should capture. scaler for wall shear
      // stress
      const auto exp_scaler_WSS =
        Scalers::NEW_exponential(sensitivity_WSS, y_0_WSS);
      // transform wall shear stress to pathway input
      auto endothelial_input_transformer =
        [inner_radius,
         flow_increase,
         exp_scaler_WSS](const dealii::Point<dim>     &p,
                         const Number                  time,
        ...
        // todo: not correct to use l2_norm since that also turns negative
        // displacement into a
        //  positive change in radius
        // determine flow rate fold change
        const double flow_rate = time > 0.0 ? flow_increase : 1.0;
        // determine radius
        const double radius =
          inner_radius +
          (displacements[0] < 0.0 ? -1.0 : 1.0) * displacements.l2_norm();
        // compute delta wall shear stress
        const double delta_WSS =
          flow_rate * std::pow(inner_radius, 3) / std::pow(radius, 3) - 1.0;

        // set wall shear stress input
        cell_state[0] = exp_scaler_WSS(delta_WSS);
      };

      //// boundary condition settings
      const double eta = 0.2; // fraction diffusing into the tissue
      const double Q_dot =
        4.5792; // Q_dot is 5.3e-14 umol/(um^2 * s) (Vaughn et al., 1998)
      // um^2 need to be changed to mm^2 -> factor of e6 and multiply by 86400
      // s/day final value should be 4.5792e-3 umol/(mm^2 * day) also equal
      // to 4.5792 nmol/(mm^2 * day) final concentrations in paraview are in
      // nmol/mm^3 (which is equal to mol/m^3) 1e-6 nmol/mm^3 are equal to 1 nM
      // (or 1e-9 mol/l)

      // production rate Q_dot * fraction diffusing into the tissue
      const double influx                         = Q_dot * eta; // 4.5792/5.;
      const double scaling_factor_for_ET_1_influx = 0.189;       // 1.0/5.3

      // create transformer for pathway input (stress and diffusion)
      auto endothelial_output_transformer =
        [influx, &scaling_factor_for_ET_1_influx](
          const std::vector<Number> &average_pathway_output,
          const std::vector<Number> &average_baseline_pathway_output,
          dealii::Vector<Number>    &boundary_values) -> void {
        // compute changes in NO production
        const auto delta_psi_NO =
          (average_pathway_output[0] - average_baseline_pathway_output[0]) /
          average_baseline_pathway_output[0];

        // compute changes in ET1 production
        const auto delta_psi_ET1 =
          (average_pathway_output[1] - average_baseline_pathway_output[1]) /
          average_baseline_pathway_output[1];

        // no influx of collagen
        boundary_values[0] = 0.0;
        // no influx of smc
        boundary_values[1] = 0.0;
        // NO
        boundary_values[2] = influx * (1.0 + delta_psi_NO);
        // ET1
        boundary_values[3] =
          scaling_factor_for_ET_1_influx * influx * (1.0 + delta_psi_ET1);
      };

      // add endothelial layer to pathway storage
      pathwayStorage.add_endothelial_cell_layer(
        Cells::CellType::EC,
        std::move(endothelial_pathways),
        endothelial_pathway_weights,
        std::move(endothelial_input_transformer),
        std::move(endothelial_output_transformer),
        endothelial_boundary_id);
