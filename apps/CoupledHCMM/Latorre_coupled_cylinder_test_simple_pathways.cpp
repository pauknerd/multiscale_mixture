...
#include <MSHCMM/pathways/equations/simple_pathway_2_outputs.h>
...
main(int argc, char *argv[])
{
  ...
  //// PARAMETERS
  const std::string output_directory = "Latorre_cylinder_simple_pathway/";
  const std::string filename =
    "Latorre_coupled_cylinder_test_simple_pathways_quadratic_s_stress_1_dP_10p";
  // prestretch
  ...
  const Number       equilibration_time            = 100.0;
  ...
  try
    {
      ...
      // create source term
      // auto source_term = std::make_shared<SourceTerm<dim>>(14.0);
      ...
      auto diffusion_transformer =
        ...
        linear_matrix[2][2] = (k_smc * (1 + delta_psi_smc) - k_smc);
        ...
        linear_matrix[1][2] =
          mass_fractions[1] / mass_fractions[2] * k_col * (1 + delta_psi_col);
      };
      ...
      // equation in the concrete class! old simple
      pathwayEquations.push_back(
        std::make_shared<Equations::SimplePathway2Outputs<Number>>());
      ...
      const auto exp_scaler =
        Scalers::exponential(sensitivity_stress, 0.0, 1.0);

      const unsigned int pathway_index_of_stress = 0;
      auto               pathway_transformer =
        [exp_scaler](const dealii::Point<dim>     &p,
                     const Number                  time,
