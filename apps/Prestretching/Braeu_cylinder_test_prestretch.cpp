...
main(int argc, char *argv[])
{
  ...
  const std::string filename = "Braeu_cylinder_test_prestretch_quadratic";
  // prestretch
  // set to high value so that displacements determine final prestretched state
  const unsigned int n_prestretch_steps = 1000;
  ...
  try
    {
      ...
      auto prestretch_strategy =
        // std::make_unique<PrestretchStrategies::PrestretchStrategyMousavi<dim,
        // Number>>();
        std::make_unique<
