...
template <int dim, typename Number = double>
class FiberPrestretch : public dealii::Function<dim, Number>
{
public:
  FiberPrestretch(const Number slope, const unsigned int n_steps)
    : dealii::Function<dim, Number>() // 1 component, initial time: 0.0
    , slope(slope)
    , n_steps(n_steps)
  {}
        ...
        const unsigned int        component = 0) const override
  {
    (void)p;
    (void)component;
    if (this->get_time() >= n_steps)
      return 1.0 + slope * n_steps;
    else
      return 1.0 + slope * this->get_time();
  }
...
main(int argc, char *argv[])
{
  ...
  //// PARAMETERS
  const std::string output_directory = "Latorre_cylinder_active/";
  const std::string filename =
    "Latorre_cylinder_test_prestretch_active_quadratic";
  // prestretch
  // set to high value so that displacements determine final prestretched state
  const unsigned int n_prestretch_steps = 200;
  // apply all the prestretches in the initial step
  const unsigned int n_load_steps = 10;
  // relative tolerance for Prestretching in terms of wall thickness
  const double prestretch_rel_tol = 1e-6;
  ...
  // time - not relevant for prestretching
  const double dt = 1.0;
  ...
  // material parameters Irons2022
  const double              pressure = 14.;
  const std::vector<Number> mass_fractions{
    0.34, 0.056 * 0.33, 0.067 * 0.33, 0.4385 * 0.33, 0.4385 * 0.33, 0.33};

  // ELASTIN
  const double E_elastin                = 89.71;
  const double axial_prestretch_elastin = std::pow(1.62, 1. / n_load_steps);
  const double kappa                    = 10.0 * E_elastin;

  // SMC
  const double k_1_smc        = 261.4;
  const double k_2_smc        = 0.24;
  const double prestretch_smc = 1.2;
  const double T_smc          = 70.;         // decay time [days]
  const double k_smc          = 0.4 / T_smc; // gain factor

  // COL
  const double k_1_col        = 234.9;
  const double k_2_col        = 4.08;
  const double prestretch_col = 1.25;
  const double T_col          = 70.;         // decay time [days]
  const double k_col          = 0.4 / T_col; // gain factor
  const double alpha          = 0.52202798;  // fiber orientation

  // active stress
  const double                      lambda_max = 1.1;
  const double                      lambda_min = 0.4;
  const double                      sigma_max  = 170.0;
  ...
  try
    {
      ...
      auto prestretch_function_smc =
        FiberPrestretch<dim, Number>((prestretch_smc - 1.0) / n_load_steps,
                                     n_load_steps);
      auto prestretch_function_col =
        FiberPrestretch<dim, Number>((prestretch_col - 1.0) / n_load_steps,
                                     n_load_steps);
      ...
      const auto cos_transformer_3 =
        Constituents::CylindricalCoordinateTransformer<dim>(
          ...
          outer_radius,
          M_PI_2 - alpha);
      // for fiber orientation of fiber 4 - helix 2
      ...
      boundary_descriptor_HCMM.add_pressure_bc(
        0,
        std::make_shared<NeumannBoundaryPrestretch<dim>>(
          0.0, -pressure / n_load_steps, n_load_steps));
